...
class CilinSimilarity(object):
    ...
    def read_cilin(self):
        ...
        所有单词保存在self.vocab
        """
        with open(self.cilin_path, encoding='utf-8') as f:
    ...
    def get_synwords(self, word):
        '''
        得到这个词的同义词列表
        '''
        if word not in self.word_code:
            return []
        synwords = []
        for code in self.word_code[word]:
            if code[-1] == '=':
                synwords += self.code_word[code]
        synwords = set(synwords)
        synwords = [_ for _ in synwords if _!=word]
        return synwords
